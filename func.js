...
function resetCanvas() {
    ...
    for (let k in OBJDATA) {
        if (OBJDATA[k].graph && typeof OBJDATA[k].graph.remove === "function") {
    ...
    zoom_maker('zoomreset', 200, 50);
    $('#boxinfo').html('Canvas cleared');
}
...
function updateHistoryButtons() {
    ...
    if (HISTORY.index < HISTORY.length) $('#redo').removeClass('disabled');
...
if (clearCanvasBtn) {
    clearCanvasBtn.addEventListener("click", function () {
        const ok = confirm('Clear the entire canvas? This removes all walls, rooms, and objects.');
        if (!ok) return;
        resetCanvas();
    });
}
...
const reportModeEl = document.getElementById('report_mode');
if (reportModeEl) reportModeEl.addEventListener("click", function () {
    ...
    for (let k in ROOM) {
        ...
        if (ROOM[k].name === "") {
            reportRoom +=
                '<div class="col-md-8"><p><i class="fa fa-ban" aria-hidden="true" style="color:red"></i> The room has no label, Top2Pano Editor cannot provide you with information.</p></div>\n';
        } else {
...
window.addEventListener("load", function () {
    ...
    const hasHistory = !!localStorage.getItem('history');
    const recoverBlock = document.getElementById('recover');
    if (recoverBlock) recoverBlock.style.display = hasHistory ? "" : "none";

    const welcomeModalEl = document.getElementById('myModal');
    if (welcomeModalEl && typeof bootstrap !== "undefined") {
        const welcomeModal = new bootstrap.Modal(welcomeModalEl, { backdrop: "static", keyboard: false });
        welcomeModal.show();
    }
...
function positionSubmenu(menu, button) {
...
const showRibEl = document.getElementById("showRib");
if (showRibEl) {
    showRibEl.addEventListener("click", function () {
        if (showRibEl.checked) {
            $('#boxScale').show(200);
            $('#boxRib').show(200);
            showRib = true;
        } else {
...
const showAreaEl = document.getElementById("showArea");
if (showAreaEl) {
    showAreaEl.addEventListener("click", function () {
        if (showAreaEl.checked) {
            $('#boxArea').show(200);
        } else {
            $('#boxArea').hide(100);
        }
...
const showLayerRoomEl = document.getElementById("showLayerRoom");
if (showLayerRoomEl) {
    showLayerRoomEl.addEventListener("click", function () {
        if (showLayerRoomEl.checked) {
            $('#boxRoom').show(200);
        } else {
            $('#boxRoom').hide(100);
        }
...
const showLayerEnergyEl = document.getElementById("showLayerEnergy");
if (showLayerEnergyEl) {
    showLayerEnergyEl.addEventListener("click", function () {
        if (showLayerEnergyEl.checked) {
            $('#boxEnergy').show(200);
        } else {
            $('#boxEnergy').hide(100);
        }
...
// distance_mode UI may be removed in this fork; keep code safe if element doesn't exist
if (document.getElementById('distance_mode')) {
...
// text_mode UI may be removed in this fork; keep code safe if element doesn't exist
if (document.getElementById('text_mode')) {
